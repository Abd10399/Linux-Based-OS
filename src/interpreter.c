...
int quit(){
	...
	//Freeing frame memory (& Shell memory???) (Do This now)
	frame_mem_free_lines_between(0, FRAME_STORE_SIZE);

	//remove backing store directory and its content
	char command[100] = "rm -r backingstore";
...
int exec(char *fname1, char *fname2, char *fname3, char* policy, bool background, bool mt){
	...
	if(fname2!=NULL){
		if(strcmp(fname1,fname2)==0){
			return badcommand_same_file_name();			//Fix this
		}
	}
	if(fname3!=NULL){		//Need to fix this to allow for similar file names
	...
	*/
	int error_code = 0;
	if(background) error_code = shell_process_initialize();				//Gives prioirty to the shell -- Makes it a process
	if(fname1 != NULL){													
        error_code = process_initialize(fname1);						//Sets up the first program as a process
		if(error_code != 0){
			return handleError(error_code);
		}
    }
    if(fname2 != NULL){
        error_code = process_initialize(fname2);						//Sets up the second program as a process and so on..
		if(error_code != 0){
			return handleError(error_code);
		}
    }
