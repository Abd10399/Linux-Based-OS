...
void terminate_process(QueueNode *node){
    ...
    for (int x = 0; x < (sizeof(node->pcb->pagetbl)/sizeof(node->pcb->pagetbl[0])); x++){
        if (node->pcb->pagetbl[x] >= 0) {

            frame_mem_free_lines_between(node->pcb->pagetbl[x]->frameNumber*3, node->pcb->pagetbl[x]->frameNumber*3 + 2);
